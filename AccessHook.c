...
HMODULE NTBASE;     // Base address of ntdll.dll    
...
NTSTATUS(_stdcall*NtProtectVirtualMemory)(HANDLE ProcessHandle, PVOID* BaseAddress, PSIZE_T NumberOfBytesToProtect, ULONG NewAccessProtection, PULONG OldAccessProtection);
NTSTATUS(_stdcall*NtQueryVirtualMemory)(HANDLE ProcessHandle, PVOID BaseAddress, MEMORY_INFORMATION_CLASS MemoryInformationClass, PVOID Buffer, SIZE_T Length, PSIZE_T ResultLength);
...
void AccessHookAdd(void* original_function_address, void* new_function_address)
{
    ...
    if (firstRun--) {
        // Intitialize Values
        NTBASE = (HMODULE)(*(**(*(uintptr_t****)(__readgsqword(0x60) + 0x18) + 0x4) + 0x4));
        NtProtectVirtualMemory = (NTSTATUS(_stdcall*)(HANDLE ProcessHandle, PVOID* BaseAddress, PSIZE_T NumberOfBytesToProtect, ULONG NewAccessProtection, PULONG OldAccessProtection))(void*)GetProcAddress(NTBASE, "NtProtectVirtualMemory");
        NtQueryVirtualMemory   = (NTSTATUS(_stdcall*)(HANDLE ProcessHandle, PVOID BaseAddress, MEMORY_INFORMATION_CLASS MemoryInformationClass, PVOID Buffer, SIZE_T Length, PSIZE_T ResultLength))(void*)GetProcAddress(NTBASE, "NtQueryVirtualMemory");
